...
async def test_admin_update_missing_user_returns_404(client, admin_headers):
    response = await client.put(
        "/api/users/99999", headers=admin_headers, json={"display_name": "Ghost"}
    )
    assert response.status_code == 404
...
class TestUnauthenticated:
    ...
    async def test_update_unauthenticated(self, client):
        resp = await client.put("/api/users/1", json={"display_name": "x"})
        assert resp.status_code in (401, 403)
