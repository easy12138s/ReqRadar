"""MVP-1 E2E smoke test — run inside api-service container."""
...
def main() -> None:
    ...
    r = httpx.post(
        ...
        json={"name": f"E2E-{uuid.uuid4().hex[:8]}", "description": "MVP-1 test", "source_type": "empty"},
        headers=h,
        timeout=30,
    )
    ...
    doc = (
        ...
        "5. Audit logging retained for 180 days\n"
        "6. Account lockout after 5 failed attempts\n"
        "7. OAuth2 third-party login support\n"
    )
    r = httpx.post(
        f"{BFF_URL}/api/v2/projects/{pid}/ingest",
        files={"file": ("req.md", doc.encode(), "text/markdown")},
        data={"project_id": pid},
        headers=h,
        timeout=120,
    )
    ...
    print(f"\nALL 9 STEPS PASSED! project={pid} session={sid} report_size={sz}")
