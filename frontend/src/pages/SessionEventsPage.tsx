...
const eventTypeColors: Partial<Record<EventType, string>> = {
  ...
  DIMENSION_UPDATED: 'orange',
  CHECKPOINT_CREATED: 'gold',
  TOOL_CALLED: 'geekblue',
};
        ...
        <Space style={{ marginBottom: 16 }}>
          <Select placeholder="事件类型" allowClear style={{ width: 200 }} value={typeFilter} onChange={setTypeFilter}
            options={[
              ...
              'STEP_STARTED', 'STEP_COMPLETED', 'EVIDENCE_COLLECTED', 'DIMENSION_UPDATED',
              'CHECKPOINT_CREATED', 'TOOL_CALLED',
            ].map((t) => ({ value: t, label: t }))}
          />
          <Select placeholder="事件级别" allowClear style={{ width: 120 }} value={levelFilter} onChange={setLevelFilter}
            options={['session', 'reasoning', 'system'].map((l) => ({ value: l, label: l }))}
          />
        </Space>
        <Table
          dataSource={filtered}
