...
TEST_DATABASE_URL = "sqlite+aiosqlite:///./test_reqradar_evidence.db"
...
async def setup_db():
    ...
    db_path = "./test_reqradar_evidence.db"
...
async def auth_client(setup_db):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        await client.post(
            "/api/auth/register",
            json={"email": "evidence@example.com", "password": "secret123", "display_name": "Evidence User"},
        )
        ...
        proj_resp = await client.post(
            ...
            json={"name": "Evidence-Project", "description": "test", "local_path": "/tmp"},
            headers=headers,
        )
