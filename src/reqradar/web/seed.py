...
async def seed_named_template(db: AsyncSession, template_name: str) -> ReportTemplate | None:
    result = await db.execute(select(ReportTemplate).where(ReportTemplate.name == template_name))
    existing = result.scalar_one_or_none()
    if existing:
        logger.info(
            "Template '%s' already exists (id=%d), skipping seed", template_name, existing.id
        )
        return existing

    loader = TemplateLoader()
    template_dir = loader.get_default_template_path().parent
    yaml_path = template_dir / f"{template_name}.yaml"

    if not yaml_path.exists():
        logger.warning("Template file not found: %s, skipping", yaml_path)
        return None

    defn = loader.load_definition(yaml_path)
    render_path = loader.get_default_render_template_path()
    ...
    logger.info("Seeded template '%s' (id=%d)", defn.name, template.id)
    return template
...
async def seed_all(db: AsyncSession):
    ...
    for name in TEMPLATE_NAMES:
        await seed_named_template(db, name)
